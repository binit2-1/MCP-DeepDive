...
server.registerTool("list_dir", {
    ...
    inputSchema: z.object({
        path: z
            .string()
            .describe(`The path to the directory to list files from. Must be within ${ALLOWED_ROOT}`),
    }),
}, async ({ path }) => {
...
server.registerTool("read_file", {
    ...
    inputSchema: z.object({
        path: z
            .string()
            .describe(`The path to the list to list files from. Must be within ${ALLOWED_ROOT}`),
    }),
}, async ({ path }) => {
    if (!path.startsWith(ALLOWED_ROOT)) {
        return {
